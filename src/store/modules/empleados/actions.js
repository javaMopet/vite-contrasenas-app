const host = "http://localhost:3000/empleados";

const getRolName = (rol)=> {
  if( rol && parseInt(rol) === 1){
...
export default {
  async requestEmpleadosList(context) {
        ...
        if (respuesta) {
          ...
          for (const id in data1) {
            results.push({
              ...
              rol: data1[id].attributes.rol,
              rolName: getRolName(data1[id].attributes.rol),
              login:  data1[id].attributes.login,
              ip: data1[id].attributes.ip,
            });
          }
          context.commit("addEmpleadosList", results);
        } else {
